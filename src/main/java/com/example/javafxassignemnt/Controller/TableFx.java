...
public class TableFx implements Initializable {
    ...
    public void TableData()
    {
        ...
        ColumnData();
        tableview1= addressController.findAll();
        for (Address address : tableview1) {

            FetchData record = new FetchData(address.getUsers().getId(), address.getUsers().getName(), address.getUsers().getEmail(), address.getUsers().getPhone(),
                    address.getAddress(), address.getTownTown().getName(), address.getTownTown().getCityCity().getName(), address.getTownTown().getCityCity().getStateState().getName(),
                    address.getTownTown().getCityCity().getStateState().getCountryCountry().getName());
            System.out.println(record);

            fetchData.add(new FetchData(address.getUsers().getId(), address.getUsers().getName(), address.getUsers().getEmail(), address.getUsers().getPhone(),
                    address.getAddress(), address.getTownTown().getName(), address.getTownTown().getCityCity().getName(), address.getTownTown().getCityCity().getStateState().getName(),
                    address.getTownTown().getCityCity().getStateState().getCountryCountry().getName()));
        }
